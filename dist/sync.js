...
var sync = function () {
    if (arguments.length > 1) {
        ...
        for (var i = 1; i < arguments.length; i++) {
            ...
            var fn = obj[fname];
            if (!fn)
                throw new Error("object doesn't have '" + fname + "' function!");
            obj[fname] = sync(fn);
        }
    }
    else {
...
sync.deferWithTimeout = function (timeout, message) {
    if (!Fiber.current)
        throw new Error("no current Fiber, deferWithTimeout can't be used without Fiber!");
    if (!timeout)
        throw new Error("no timeout provided!");
    if (Fiber.current._syncParallel)
        throw new Error("deferWithTimeout can't be used in parallel!");
...
sync.deferParallel = function () {
    ...
    if (!fiber)
        throw new Error("no current Fiber, defer can't be used without Fiber!");
    if (!fiber._syncParallel)
        throw new Error("invalid usage, should be called in parallel mode!");
    ...
    return function (err, result) {
        ...
        nextTick(function () {
            ...
            if (err) {
                ...
                if (data.returned == data.called)
                    fiber.run(data.results);
            }
        });
    };
};
...
sync.asyncIt = function (cb) {
    if (!cb)
        throw "no callback for async spec helper!";
        ...
        for (var i = 1; i < arguments.length; i++) {
            (function (fname) {
                ...
                obj[fname] = function () {
                    ...
                    var indent = '    ';
                    if (Fiber.current)
                        for (var j = 0; j < Fiber.current._callbackLevel; j++)
                            indent = indent + '  ';
